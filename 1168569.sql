...
SELECT DISTINCT username,
	   COUNT(*),
       type.title
 FROM Player INNER JOIN Phonemon ON Player.id = Phonemon.player INNER JOIN Species ON Species.id = Phonemon.id INNER JOIN type ON type.id = species.type1 
 WHERE type.title IN (SELECT type.title FROM Type INNER JOIN Species WHERE Type.id = Species.type1 GROUP BY title ORDER BY COUNT(*) DESC)
 GROUP BY username, type.title;
-- END Q10
-- ____________________________________________________________________________________________________________________________________________________________________________________________________________
